...
struct Emissions::Impl {
  Impl(std::string_view emissions)
      : string_repr_("NaN"), precision_(0), multiprecision_repr_(0) {
    const char *const start = emissions.data();
    const char *const end = start + emissions.size();
    ...
    if (dot_pos == end) {
      ...
      // build a copy of emissions string, starting with nonzero digit and having
      // no decimal separator
      std::string emissions_without_dot;
      emissions_without_dot.append(first_nonzero_digit_pos, dot_pos);
      emissions_without_dot.append(dot_pos + 1, end);
      try {
        multiprecision_repr_ =
            boost::multiprecision::checked_uint256_t(emissions_without_dot);
      } catch (std::overflow_error const &) {
        return;
      }
      string_repr_.clear();
...
Emissions::Emissions(std::string_view emissions)
    : impl_(std::make_unique<Impl>(emissions)) {}

Emissions::Emissions(types::PrecisionType precision)
    : impl_(std::make_unique<Impl>(precision)) {}

Emissions::Emissions(Emissions const &other)
    : impl_(std::make_unique<Impl>(*other.impl_)) {}

Emissions::Emissions(Emissions &&other) noexcept
    : impl_(std::exchange(other.impl_, nullptr)) {}

Emissions &Emissions::operator=(Emissions const &other) {
  return *this = Emissions(other);
}

Emissions &Emissions::operator=(Emissions &&other) noexcept {
...
int Emissions::sign() const {
  return impl_->multiprecision_repr_.sign();
}

types::PrecisionType Emissions::precision() const {
  return impl_->precision_;
}

std::string const &Emissions::toStringRepr() const {
...
bool Emissions::operator==(const ModelType &rhs) const {
...
std::string Emissions::toString() const {
