...
def hash_part_tree(peer, peers, root_partid):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            sql_ = f"""
                INSERT INTO temp (partid, parents_partid, priority, assembler, cfp, co2) 
                WITH cfpval AS (
                    {co2_import}
                ),
                part_tree AS( 
            ...
            sql_1 = """
                ...
                UPDATE hashvals SET can_hashing = 't'
                    WHERE NOT EXISTS (SELECT r.parents_partid FROM partrelationship r, temp WHERE r.parents_partid = temp.partid);
            ...
            while True: 
                ...
                sql_2 = """
                    WITH get_can_hashing_part AS (
                        SELECT parents_partid as partid, bool_and(can_hashing) as result, array_agg(hash) AS hash_list
                        FROM hashvals, temp
                        WHERE temp.parents_partid = hashvals.partid
                        Group by parents_partid
                    ),
                    update_hash AS (
                        UPDATE hashvals SET hash = array_cat(hash, hash_list)
                        FROM get_can_hashing_part r1
                        WHERE r1.partid = hashvals.partid AND result = 't'
                    )
                    UPDATE hashvals SET (can_hashing, hash) = ('t', ARRAY[xor_sha256(hash)]);
                """ 
                cur.execute(sql_2)
